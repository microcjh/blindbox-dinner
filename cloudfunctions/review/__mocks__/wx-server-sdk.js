// mock: 模拟 wx-server-sdk（review 云函数 + common/db 用到的能力）
// 与 match/register/events/payment mock 同范式：collection(name) 返回**共享 ctx 的链对象**，方法全部 return this。
// 支持 review 用到的查询：字面量等值 / $ne / $in / 数组包含（match_groups.members 含 uid）/ 分页 / 排序 / 字段裁剪。
...
function matchOne(r, k, v) {
  ...
  // 数组字段「包含」匹配（members 含 uid）
  if (Array.isArray(r[k])) return r[k].includes(v);
  return r[k] === v;
}

// 等值 where 匹配（仅支持字面量等值 / $ne / $in / 数组包含）
