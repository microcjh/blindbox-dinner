// mock: 模拟 wx-server-sdk（match 云函数 + common/db 用到的能力）
// 与 register/events/payment mock 同范式：collection(name) 返回**共享 ctx 的链对象**，方法全部 return this。
// 额外支持 match 用到的查询操作符：$ne（不等）与数组字段 members 的「包含」匹配、$in（in 集合）。
