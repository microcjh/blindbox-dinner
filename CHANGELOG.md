...
- **管理后台审核台 UI（task-028）**：补齐 admin 真实审核台页面（task-023 门面+单测已就绪）。新增 `subpackages/admin/pages/console`——`举报审核`（listReports + handleReport resolved/banned）+ `SOS 处置`（listSos + handleSos）双 Tab，含骨架/空态/下拉刷新；**403 整页切「无权限」占位**（云端 requireAdmin 才是真闸门，前端不信赖判定）。`app.json` 注册 `subpackages`；`pages/profile` 底部加「管理后台」入口（`goAdmin`）。`coding-style.md` 新增 §30。
- **饭局评价页接通（task-029）**：`pages/review` 由占位"邀请函"接通为真实"饭局评价/沉淀"页——`listReviews(eventId)`（浏览类 loading:false）展示本场全部评价（匿名化 `饭友 xxxx`）+ 对同桌饭友 `submitReview`（评分 1–5 + 预设标签 + 可选评论，走默认 loading）；`reviewedUids` 本地回显"已评"、云端 409 为权威。`event-detail`「饭后沉淀」卡片加"查看全部评价 ›"入口（`goReview` → navigateTo 透传 eventId + 逗号 uid）。`coding-style.md` 新增 §31。
- **SOS 一键求助页接通（task-030）**：`pages/sos` 由占位"建设中"接通为真实求助页——表单含 4 类求助选择（`{value,label,desc,on}` 对象数组驱动，白名单 `unsafe/lost/medical/other` 与云函数 `VALID_TYPES` 一致）+ 选填描述（≤200）+ 可选位置（`wx.getLocation` 失败降级不带位置）+ 二次确认提交（`createSos` 默认 loading）；`loadMine` 调 `mySos`（浏览类 loading:false）展示"我的求助记录"（状态 pending=待处置 / handled=已处置 + 时间）；无 `eventId` 横幅禁用提交、未登录切 `ui-empty`（去登录）。`event-detail` 的 SOS 浮标由内联 `createSos(type:'unsafe')` 改为 `navigateTo /pages/sos/sos?eventId=..`（类型选择/二次确认移入 sos 页，浮标回归"一键跳转"）。`coding-style.md` 新增 §32。
- **邀请分享页接通（task-031）**：`pages/invitation` 由占位"盲盒问卷·建设中"接通为真实"邀请好友"分享页——**纯前端微信原生分享、不依赖自建后端**：好友分享 `onShareAppMessage`（`<ui-button open-type="share">` 透传触发，视觉与全仓一致）+ 朋友圈 `onShareTimeline`（仅右上角「···」触发，UI 用 `ui-tag` 提示"点右上角"）+ 复制邀请语（`wx.setClipboardData`）；`onLoad` 调 `wx.showShareMenu({menus:['shareAppMessage','shareTimeline']})`。`path/query` 带 `inviter=<uid>&from=invite` 透传（落地首页读取），裂变统计（邀请关系/奖励）待后端补；"我的分享记录"存本地 `invite_records`（仅本人行为，不涉及他人数据，合规）。`coding-style.md` 新增 §33。
